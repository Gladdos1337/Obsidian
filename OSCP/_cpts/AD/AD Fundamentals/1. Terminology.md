...
- ID of an every single item in AD - never changes, eg. being User with ID 123, if the username is changed or user gets promoted, the ID still stays 123.
...
![[Pasted image 20260704204524.png]]
...
- **`NTDS.DIT`:** This is the holy grail. If you get Domain Admin, you dump this file to get every password hash in the network.
    
- **`adminCount = 1`:** When you run BloodHound or BloodHound.py during enumeration, you filter for this. If a normal-looking user has `adminCount = 1`, it means they belong (or used to belong) to a high-privilege group.
    
- **`sIDHistory`:** If an admin migrated accounts poorly, old high-privilege SIDs might be hiding here, allowing you to escalate privileges.
    
- **`SYSVOL`:** You will systematically search this shared folder for poorly hidden passwords in setup scripts or old Group Policy preference files (`Groups.xml`).
    
- **SPN (Service Principal Name):** This is the trigger for **Kerberoasting**. If a user account has an SPN, you can request a ticket for it and crack its password offline.